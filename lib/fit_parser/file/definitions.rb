  ...
  class File
    module Definitions
      ...
      def self.add_field(global_msg_num, field_def_num, name, options = {})
        if Definitions.fields[global_msg_num].key?(field_def_num)
          raise "bad definition of dynamic field  (#{name}) without :ref_field_name or :ref_field_values" unless options.key?(:ref_field_name) && options.key?(:ref_field_values)
